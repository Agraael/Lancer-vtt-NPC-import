...
function _badge(status, count) {
    let text = "";
    if (status === "synced")        text = count > 1 ? `✓ (×${count})` : "✓";
    else if (status === "modified") text = count > 1 ? `⚠ (×${count})` : "⚠";
    else if (status === "new")      text = "+";
    if (!text) return "";
...
function _mechCounts(mech) {
    ...
    for (const mount of (loadout?.weapon_mounts || []))
        for (const slot of (mount.slots || []))
            if (slot.weapon?.id || slot.weapon?.lid) weapons++;
...
function _mechRow(mech, pilotIndex, mechIndex) {
    ...
    if (weapons > 0) detailParts.push(`${weapons}w`);
    if (systems > 0) detailParts.push(`${systems}sys`);
    if (stats.hp)        detailParts.push(`HP ${stats.hp}`);
    if (stats.structure) detailParts.push(`STR ${stats.structure}`);
    if (stats.stress)    detailParts.push(`STS ${stats.stress}`);
...
export function buildPilotSelectionPanel(pilots) {
    ...
    function getSelected($root) {
        ...
        $root.find(".mech-checkbox:checked").each(function() {
            ...
            if (Number.isNaN(pIdx) || !pilots[pIdx]) return;
